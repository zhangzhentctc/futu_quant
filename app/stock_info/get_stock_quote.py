...
class get_stock_quote(threading.Thread):
    ...
    def init_ticker_tl(self):
        ...
        #        'sequence':0
        #    })
        col_list = ['Ticker', 'stock_code', 'time', 'price', 'volume', 'turnover', "ticker_direction", 'sequence']
        self.ticker_tl = pd.DataFrame(data, columns=col_list)
        self.ticker_tl_old = pd.DataFrame(data, columns=col_list)
        self.ticker_new_list = []
    ...
    def process_new_tickers(self):
        ...
        if self.ticker_buff.mutex_lock.acquire(1):
            ...
            for i in range(0, len(self.ticker_new_list)):
                list.append(self.ticker_new_list[i])
    ...
    def cal_new_tickers(self):
        ...
        if first_match == -1:
            ...
            for i in range(self.ticker_num - first_match, self.ticker_num):
                time = self.ticker_tl["time"][i]
                price = self.ticker_tl["price"][i]
                volume = self.ticker_tl["volume"][i]
                ticker_direction = self.ticker_tl["ticker_direction"][i]
