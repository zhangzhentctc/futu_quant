...
###########Oprations on table samples
    ...
    def dbop_update_sample_status(self, db_basic, id, status):
        sql = "update samples set status = " + str(status) + " where id = " + str(id) + ";"
        ret = db_basic.insertMysql(sql)
        #print(sql)
        return ret

    def dbop_update_sample_status_all_enable(self, db_basic):
        sql = "update samples set status = 1;"
        ret = db_basic.insertMysql(sql)
        #print(sql)
        return ret

    def dbop_update_sample_status_all_disable(self, db_basic):
        sql = "update samples set status = 0;"
        ret = db_basic.insertMysql(sql)
        #print(sql)
        return ret