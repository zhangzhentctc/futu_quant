...
class store_control(threading.Thread):
    ...
    def run(self):
        db = MySQLCommand("localhost", 3306, "root", "123456", "matrend")
        db.connectMysql()
        op = dbop_ma_trand()
        while(1):
            ...
            op.dbop_store_ma_dur2(db, ma10_rate, ma20_rate, cur_val)
            ...
            cur_val = self.detectMATrend.get_cur_val()
            op.dbop_store_ma_dur5(db, ma10_rate_5, ma20_rate_5, cur_val)
            time.sleep(0.5)
